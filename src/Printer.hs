...
instance PP [Int] where
  pp arr = PP.brackets $ PP.hsep $ PP.punctuate PP.comma $ map pp arr
...
instance PP Statement where
  ...
  pp (Decl binding expr) =
    PP.hsep
      ...
      <> PP.semi
  pp (Assert pred) =
    PP.hsep [PP.text "assert", pp pred] <> PP.semi
  pp (Assign var expr) = PP.hsep [pp var, PP.text ":=", pp expr] <> PP.semi
  pp (If cond thenBlock elseBlock) =
    let elsePretty = pp elseBlock
     in let withoutElse = PP.text "if" <+> pp cond <+> PP.char '{' $+$ PP.nest 4 (pp thenBlock) $+$ PP.char '}'
         in if PP.isEmpty elsePretty
              then withoutElse
              else
                withoutElse
                  $+$ PP.text "else" <+> PP.char '{'
                  $+$ PP.nest 4 (pp elseBlock)
                  $+$ PP.char '}'
  pp (While inv cond body) =
    PP.text "while"
      <+> pp cond
      $+$ PP.nest 2 (PP.text "invariant" <+> pp inv)
      $+$ PP.char '{'
      $+$ PP.nest 4 (pp body)
      $+$ PP.char '}'
...
instance PP Method where
  pp (Method name params outputs specs body) =
    let specsPretty = (PP.hsep $ PP.punctuate PP.comma $ map pp outputs)
     in PP.hsep
          [ PP.text "method",
            PP.text name
              <> PP.parens (PP.hsep $ PP.punctuate PP.comma $ map pp params),
            if PP.isEmpty specsPretty
              then PP.empty
              else
                PP.text "returns"
                  <+> PP.parens specsPretty
          ]
          $+$ PP.nest 2 (PP.vcat $ map pp specs)
          $+$ PP.char '{'
          $+$ PP.nest 4 (pp body)
          $+$ PP.char '}'

-- Helper instance to print specifications
instance PP Specification where
  pp (Requires pred) = PP.text "requires" <+> pp pred
  pp (Ensures pred) = PP.text "ensures" <+> pp pred
  pp (Modifies name) = PP.text "modifies" <+> pp name
